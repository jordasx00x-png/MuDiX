...
import { InteractiveTilt, MagneticButton } from './DynamicEffects';
...
export function FloatingParticles({ theme }: { theme?: string }) {
  ...
  const particles = useMemo(() => {
    return [...Array(isXV ? 70 : 50)].map((_, i) => ({
      size: Math.random() * (isXV ? 5 : 8) + 2,
        ...
        return (
          <motion.div
            ...
            className={cn(
              ...
              isXV ? "bg-white/40 shadow-[0_0_20px_rgba(255,255,255,0.8)]" : "bg-white/20"
            )}
            style={{
              width: p.size + 'px',
              height: p.size + 'px',
        ...
        {data.guestName && (
          <GlassCard theme={theme} className="mb-16 mx-4 md:mx-0">
            ...
            <div className="inline-flex items-center justify-center gap-4 px-8 py-4 rounded-full bg-white/5 border border-white/10 shadow-inner">
              ...
              <span 
                className={cn("text-2xl font-bold", !theme.accentColor && theme.accent)}
